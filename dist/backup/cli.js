...
function resolveKey(args, env) {
  if (args.keyFile) return (0, import_node_fs.readFileSync)(args.keyFile, "utf8").trim();
...
function openSource(path) {
  return path === "-" ? process.stdin : (0, import_node_fs.createReadStream)(path);
}
...
`);
  ...
  if (args.command === "restore") {
    if (args.toDatabase) {
      ...
      psql = (0, import_node_child_process2.spawn)("psql", ["--quiet", "--no-psqlrc"], {
        env: { ...process.env, ...connectionEnv(url) },
        stdio: ["pipe", "inherit", "inherit"]
      });
      sink = psql.stdin;
    } else if (args.out) {
      sink = args.out === "-" ? process.stdout : (0, import_node_fs.createWriteStream)(args.out);
    } else {
